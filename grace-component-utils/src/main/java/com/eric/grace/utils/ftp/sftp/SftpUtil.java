...
public class SftpUtil {
    ...
    public static ChannelSftp getSftpConnect(String ftp_file_name) throws JSchException {

        if (!StrUtil.isBlank(ftp_file_name)) {
            Props props = new Props(ftp_file_name);
            host = props.getStr("grace.sftp.host");
            port = props.getInt("grace.sftp.port");
            username = props.getStr("grace.sftp.username");
            password = props.getStr("grace.sftp.password");
            timeout = props.getInt("grace.sftp.timeout");
            prefix = props.getStr("grace.sftp.prefix");
            logger.info("host={},port={},username={},password={},timeout={},prefix={}", host, port, username, password,timeout,prefix);
        } else {
...
//        }
     ...
     * @throws SftpException
     */
    public static void exeRmRec(final String pathString, final ChannelSftp sftp) throws SftpException {
    ...
    public static List<String> formatPath(final String srcPath) {
        ...
        dir = prefix + (dir.length() == 1 ? dir : (dir + "/"));
        list.add(dir);
        list.add(fileName);
        return list;
    }
